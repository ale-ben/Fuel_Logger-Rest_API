...
const inPath = "/Users/aleben/LocalDocuments/Git/Fuel_Logger/restApi/tools/in.txt"
const outPath = "/Users/aleben/LocalDocuments/Git/Fuel_Logger/restApi/tools/out.json"
const errPath = "/Users/aleben/LocalDocuments/Git/Fuel_Logger/restApi/tools/err.txt"

function parse(strArr) {
    const res = {};

    strArr.forEach(item => {
        if (item.includes("km")) res.odometer = item.replaceAll("km", "");
        else if (item.includes("€/l")) res.relPrice = item.replaceAll("€/l", "");
        else if (item.includes("€")) res.price = item.replaceAll("€", "");
        else if (item.includes("l")) res.amount = item.replaceAll("l", "");
    ...
    if (
        ...
        if (calcRelPrice.toFixed(3) === parseFloat(res.relPrice).toFixed(3)) return res;
        else {
